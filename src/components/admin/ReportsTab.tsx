...
type ReportOrder = {
  id: string;
  receipt_no: string | null; // ✅ 1. เพิ่ม field นี้
  created_at: string;
  total_price: number;
...
export default function ReportsTab() {
  ...
  const fetchReport = async (date: string) => {
    ...
    // ✅ 2. เพิ่ม receipt_no ในคำสั่ง select
    const { data } = await supabase
      .from("orders")
      .select(`
        id, receipt_no, total_price, created_at, 
        tables(label), 
  ...
  const handleExport = () => {
    ...
    // ✅ 3. เพิ่มคอลัมน์ Receipt No. ใน CSV
    let csv = "Receipt No.,Date,Time,Table,Items,Total\n";
    ...
    reportData.forEach(o => {
      ...
      csv += `${receipt},${date},${time},${o.tables?.label || '-'},"${items}",${o.total_price}\n`;
    });
