...
export default function SettingsTab() {
  ...
  const fetchData = async () => {
    ...
    if (settings) { setShopName(settings.shop_name||""); setPromptPayId(settings.promptpay_id||""); setShopLogo(settings.shop_logo_url); }
  ...
  const handleCleanup = async () => {
    ...
    if (prompt("พิมพ์ DELETE เพื่อยืนยัน") !== "DELETE") return;
    
    setIsCleaning(true);
    try {
        const cutOff = new Date(cleanupDate).toISOString();
        const { data: oldOrders } = await supabase.from('orders').select('id').lt('created_at', cutOff).in('status', ['completed', 'cancelled']);
        ...
        alert("✅ ล้างข้อมูลเก่าเสร็จสิ้น"); setCleanupDate("");
    ...
    <div className="space-y-8">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-6">
            ...
            <div><label className="font-bold text-sm text-gray-600 block mb-1">โลโก้</label><input type="file" accept="image/*" onChange={e=>{if(e.target.files?.[0])setLogoFile(e.target.files[0])}} className="w-full border p-2 rounded text-sm bg-gray-50"/>{shopLogo&&!logoFile&&<img src={shopLogo} className="h-16 mt-2 border p-1 rounded"/>}</div>
            </div>
